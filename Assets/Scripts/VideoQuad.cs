...
internal class VideoQuad
{
    ...
    public void PanTiltZoom(float pan, float tilt, float zoom, bool corrective_depth)
    {
        _pan = Mathf.Clamp(pan, -_max_pan, _max_pan);
        _tilt = Mathf.Clamp(tilt, -_max_tilt, _max_tilt);
        _zoom = Mathf.Clamp(zoom, -_max_zoom * 0.25f, _max_zoom * 2);
        ...
        Vector3 offset = corrective_depth ? ((tilt_indent + pan_indent + _zoom) * _camera.transform.forward) : _zoom * _camera.transform.forward;// - ((tilt_offset * _camera.transform.up) + (pan_offset * _camera.transform.right)) * 0.25f;
